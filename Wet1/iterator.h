...
public:
	...
	explicit Iterator(Node<key_t, data_t>* node_ptr, int order = 0) noexcept: ptr(node_ptr), order(order) {}
	~Iterator() {}
...
template<typename key_t, typename data_t>
Iterator<key_t, data_t>& Iterator<key_t, data_t>::next(){
	Node<key_t, data_t>* parent;
